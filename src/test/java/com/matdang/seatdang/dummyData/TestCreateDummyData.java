...
public class TestCreateDummyData {
    ...
    public void Test1(String email, String memberName) {
        ...
        Customer customer = Customer.builder()
                ...
                .customerProfileImage(
                        "https://kr.object.ncloudstorage.com/myseatdang-bucket/profile-images/1/abstract-user-flat-4.png")
                .build();
        //when
    ...
    @Test
    @DisplayName("웨이팅 오늘 데이터 100만건 생성")
    void createWaitingTodayBy1_000_000()throws InterruptedException  {
        //321351
        // 238113 + 8163
        // 210000
        // 210605
        ...
        for (int i = 0; i < threadCount; i++) {
            executor.submit(() -> {
                try {
                    for (long j = 0; j < 1000000 / threadCount; j++) {
                        Customer mockCustomer = Customer.builder()
                                .memberId(ThreadLocalRandom.current().nextLong(1, 100001))
                                .memberPhone(generateRandomKoreanPhoneNumber())
                                .build();
                        when(authService.getAuthenticatedMember()).thenReturn(mockCustomer);
                        redissonLockWaitingCustomerFacade.createWaiting(ThreadLocalRandom.current().nextLong(1, 1001),
        ...
        executor.shutdown(); // 작업 완료 후 종료
                    ...
                    if (value != com.matdang.seatdang.waiting.entity.WaitingStatus.WAITING) {
                        ...
                        for (int j = 0; j < 20; j++, i++) {
                            waitingStorageRepository.save(WaitingStorage.builder()
                                    ...
                                    .storeId(1L)
                                    .customerInfo(
                                            new CustomerInfo(1L, "010-1111-1111", ((int) (Math.random() * 3 + 1))))
            ...
            public void test2 () {
                //given
                String[] desc = {"웨이팅을 받는 웨이팅 전용 매장입니다", "사전 주문 케이크만 판매 합니다. 예약 후 방문 해주세요",
                        "특별한 날에 걸맞는 특별한 케이크를 준비 해드립니다"};
